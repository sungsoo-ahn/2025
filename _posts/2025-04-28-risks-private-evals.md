...
- **User-Driven Evaluation**: The users can compare two LLMs side by side on the same user-written prompt. Subsequently, the users then rank which model they believe performed better, contributing to an ELO score for each model in the public leaderboard.
- **Dynamic and Diverse**: This method allows for a more dynamic and diverse evaluation process, as real users provide feedback on a wide range of prompts.
...
ScaleAI recently introduced the SEAL leaderboard as a step towards standardizing privatized leaderboards to provide high-quality, unbiased evaluations by controlling the entire evaluation pipeline. By leveraging proprietary datasets and expert annotators, these companies strive to mitigate issues of data contamination and bias that plague open evaluation methods.
...
Private leaderboards managed by data curation companies introduce a new set of risks that can have significant implications for the AI community, and may jeopardize the reliability of such leaderboards. These risks revolve around financial incentives, potential conflicts of interest, and various forms of evaluation bias that can skew the assessment of language models.
...
To empirically demonstrate how evaluator biases can influence private evaluations, we conducted an experiment simulating two private evaluation companies, Company Alpha and Company Beta. Each company develops its own set of evaluation leaderboards but has the dual responsibility of also providing instruction fine-tuning data to other LLM trainers. This setup mirrors real-world scenarios where Company Alpha and Beta might represent ScaleAI and LMSys respectively.
...
- Model B: The same Mistral fine-tuned on outputs provided by Company Beta (GPT-4o).


The experiment followed these steps:

1. **Collecting Annotator Answers**
We began with the Alpaca Human Preference Dataset <d-cite key="taori2023stanford"></d-cite>, which includes pairs of outputs for the same query generated by different models.
   - We presented the questions from the dataset to the annotators
   - Collected answers from each evaluator

2. **Fine-Tuning Models**
Using the preference-completion data:
   - Model A trained on Company Alpha answers
   - Model B trained on Company Beta answers

3. **Generating Outputs**
   - Both models generated responses to 805 queries
   - Used Alpaca Farm Evaluation Dataset <d-cite key="tatsulab2023alpaca"></d-cite>

4. **Evaluation**
   - Both Evaluator Alpha and Evaluator Beta independently assessed the outputs of both models for each query.
   - Recorded preferences for each output pair
...
$$
\text{Self Bias}_A = \frac{\text{(Judge A Prefers M_A- Judge B Prefers M_A)}}{\text{Average # of Preferences by Judges A and B for M_A}} \times 100
$$
...
An ELO difference of 44 on the LMSys leaderboard is a significant amount of bragging rights, as can be seen by these tweets at a 5 ELO difference. 
