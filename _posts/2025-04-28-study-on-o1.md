...
## Technical Route Deduction of OpenAI-o1

At present, the OpenAI-o1 appears to follow a similar trajectory to STaR and Quiet-STaR, focusing on optimizing the generation process of implicit Chains of Thought (CoT). The key question, then, is how to construct an effective reward mechanism for optimizing the implicit CoT generation process. Our reasoning is as follows:

* Unlike traditional tree-search approaches that optimize the output step-by-step, o1 aims to optimize the model's internal generation of implicit CoT. In this framework, tree-search nodes are no longer individual tokens of the final answer but represent intermediate steps in the implicit reasoning process.
* The optimization focuses on the adversarial dynamics between the Generator and Verifier during the reasoning process, ensuring both components collaboratively refine the quality of the implicit CoT.
* A Process Reward Model is introduced to address the challenges of long-range dependency in implicit CoT, ensuring consistency and coherence across extended reasoning processes.
* A Critic Model is incorporated to tackle the difficulty of generating reasonable reasoning processes for highly complex tasks, particularly when the model itself struggles to produce valid intermediate reasoning steps.
* The reasoning tokens in the generation process are dynamically introduced, which minimizes unnecessary computational overhead caused by extraneou.

Below is the figure which shows how the o1 model works:

{% include figure.html path="assets/img/2025-04-28-study-on-o1/principle.png" class="img-fluid rounded z-depth-1" style="width: 70% !important; height: auto;" %}

The o1 models utilize unique "reasoning tokens" to facilitate their thinking process. They analyze the question, explore various response strategies, and then generate an answer. After producing the answer, the reasoning tokens are discarded, leaving only the final response.
