...
toc:
  - name: MiniLLM - Knowledge Distillation of Large Language Models
    subsections:
    - name: Background
    - name: Intention
...
**Large Language Models (LLMs)** have witnessed a surge in computational demand as they've grown in size. <!--TODO: Add statistics about model growth and computational requirements-->
...
With white-box access to the teacher model, the **miniLLM approach** can poll the _student_'s output distribution to mimic $$q_\theta$$, the frequency of responses from the student, and compare it with the true value of $$p$$ extracted from the teacher to propagate $$\nabla\mathcal{L}(\theta) = \nabla D_{\text{KL}}(q_\theta||p)$$. In intuitive terms, this means instead of increasing the likelihood the student outputs the full range of samples frequently outputted by the teacher, we aim to train the student to output samples that resonate with the teacher's preference/distribution.

Let's say that we fed in an input prompt $$\bm{x}$$ and we want to predict the token at position $$t'$$ given the tokens up to that point, $$\bm{y}_{<t'}$$. Given this token, $$y_{t'}$$ we can examine how well the student model distills knowledge from the teacher model by comparing their probability of outputing this token: 
$$r_{t'} = \log \frac{p(y_{t'} | \bm{y_{<t'}, \bm{x}})}{q_\theta (y_{t'} | \bm{y_{<t'}, \bm{x}})}$$
...
$$R_t = \log\left( \frac
{\prod_{t'=t}^{T} p(y_{t'} | \bm{y_{<t'}, \bm{x}}) }
{\prod_{t'=t}^{T} q_\theta (y_{t'} | \bm{y_{<t'}, \bm{x}}) } 
\right) =
\sum_{t'=t}^{T} r_{t'}
$$

For our setup, say we repeatedly sample input prompts $$\bm{x}$$ from some distribution of input prompts $$p_{\bm{x}}$$ (labeled as $$p$$ because it is a natural language distribution that should closely mirror the teacher's language distribution of outputs). We then feed this through the student model with its current parameters $$\theta$$ and get an output distribution $$q_\theta(\cdot | \bm{x})$$. 

We adapt this thinking into a **Policy Gradient**<d-cite key="Williams1992PolicyGradient"></d-cite> formulation to show the feedback we will give the student model (after giving it an input prompt $$\bm{x}$$ and receiving an output $$\bm{y}$$):
$$
\nabla \mathcal{L}(\theta) = - 
  \underset{\substack{
  \bm{x} \sim p_{\bm{x}} \\
  \bm{y} \sim q_\theta(\cdot | \bm{x}) }}
  {\mathbb{E}}
\sum_{t=1}^T (R_t - 1) \nabla\log q_\theta (y_t | \bm{y}_{<t} , \bm{x}) 
$$
Some things to note about this formulation:
- Instead of relying on single samples to calculate our loss, we can calculate the expected value across samples from the input distribution $$p_{\bm{x}}$$ and the corresponding output distribution $$q_\theta(\cdot | \bm{x})$$ for each such sample $$\bm{x}$$.
- We accumulate the loss over each token in our $$T$$-length output.
- As an high-level example, if $$R_t - 1$$ is very low (very negative), this means probability $$p$$ was much lower than $$q$$ through point $$t$$, and our student is over-producing an output that is insignificant in the teacher's distribution. Multiplied with the negative sign from outside the $$\mathbb{E}$$, this attaches a high positive multiplier so that $$\nabla\mathcal{L}(\theta) \propto \nabla\log q_\theta (y_t | \bm{y}_{<t} , \bm{x})$$. Thus, to minimize loss (and traverse in the direction of $$-\nabla\mathcal{L}(\theta)$$), we would want to decrease $$\log q_\theta (y_t)$$ and thus decrease $$q_\theta (y_t)$$ so it gets closer to $$p (y_t)$$ and is less likely to output the token $$y_t$$ in the given sequence.
...
1. **Single-Step Decomposition**: to reduce variance (spikes in the training graph), the authors sought to split the single-step loss variable $$r_t$$ while still keeping the accumulated loss $$R_t$$ so that individual loss items could be calculated separately (TODO and thus efficiently?) which helped accelerate and smoothen convergence:
$$
\nabla\mathcal{L}(\theta)
...
- \mathbb{E} 
  ...
  \right)
$$
TODO: for the single term, how is $$\bm{y}$$ sampled from the student output but also $$y_t$$ is sampled separately in the nested sum?
<!--TODO: isn't summing over the full vocabulary inefficient? how big is the vocabulary?-->
<!--TODO: do we even bother with the derivation of this-->

2. **Teacher-Mixed Sampling**: A common problem in training with reinforcement policies is **Reward Hacking**<d-cite key="Skalse2024RewardHacking"></d-cite>, wherein the student attempts to minimize its own loss function, which is just a _proxy_ for the true, abstract mission of just aligning with the natural language distribution however possible. This could lead to the student finding loopholes to minimize its loss by the "letter of the law", rather than the "spirit of the law". In practice, this could mean short, degenerate, or repeated phrases that happen to have a low loss value. To prevent this, rather than evaluating samples $$\bm{y}$$ drawn purely from the student's (potentially reward-hacking) distribution $$q_\theta$$, we draw them from a weighted combination
$$\tilde{p} = \alpha p + (1-\alpha) q_\theta$$, 
where $$\alpha = 1/5$$. With this modification, we need to adjust the loss propagated to the student so it only gets loss proportional to its share of the final distribution, so we change our formula to add a weighting of $$w_t = \frac{q_\theta (y_t | \bm{y}_{<t} , \bm{x})}{\tilde{p} (y_t | \bm{y}_{<t} , \bm{x})}$$:
$$
\nabla\mathcal{L}(\theta)
= - \underset{\substack{
  \bm{x} \sim p_{\bm{x}} \\ 
  \bm{y} \sim \tilde{p}(\cdot | \bm{x})}}
  {\mathbb{E}} 
\left( \sum_{t=1}^T w_t 
...
### Methods
<!--TODO info about types and sizes of models used, datasets used, etc various models, 120M -> 13B size, evaluated on 5 datasets (list them?) split by response length. OpenWeb for GPT2 and RoBERTa training for other models. mention these in terms of $$\mathcal{D}_{\text{PT}}$$ and $$\mathcal{D}$$ for consistency-->

The resulting student would be trained via the iterative KD loop described previously, with the addition of clipping for added stability. <!--TODO summarize what that means by referencing https://arxiv.org/pdf/1707.06347-->

The authors implemented MiniLLM with multiple models and sizes.
- **GPT-2** (<!--TODO cite-->)
...
- **SFT w/o KD**: Students fine-tuned directly on $$\mathcal{D}$$ without KD, but with the added feedback of **golden responses** <!--TODO what are these?-->
- **KD**: Students trained via word-level feedback from teacher's $$p$$ (<!--TODO does this use forward or reverse KLD-->)
- **SeqKD**: Students trained on input-output data generated by a teacher model (<!--TODO does this use forward or reverse KD, also what does this mean exactly-->) 
...
- GPT-4 evaluation: the authors fed the outputs from MiniLLM and a comparison point to GPT-4 and asked the bigger model to evaluate which response was of higher quality (TODO fact check this I actually didn't read that part)
