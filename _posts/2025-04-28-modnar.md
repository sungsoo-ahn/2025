...
- `k_reset` never changes -- again by design; see above
...
were too hard to train, getting many vanishing gradients</d-footnote>, but
I have found that the simplest one worked best.  You might have guessed it from
...
key="jiang2024mixtral"></d-cite> where the output for a given input $x$ is
determined by the weighted sum of outputs of some expert networks. Experts
...
<div class="caption">
    Two experts choosing what inputs to attend to.
</div>
</div>

Deciding which variable (inputs or hint) to route to which expert is based on
the latent graph embedding of each expert, as shown in the above image. For
each expert, once a graph embedding is obtained, a linear projection with
a sigmoid activation gives the attention coefficient in $(0,1)$ representing
the importance of a variable to the expert. The graph embedding I used here is
based on PrediNet <d-cite key="shanahan2020anexplicitly"></d-cite>, since mean
pooling will provably not generalise <d-cite
key="adam-day2024almost"></d-cite> and max pooling gives sparse gradients.
...
experts' latents is **added**<d-footnote>This creates a residual connection. If only the weighted sum of the two is used, training gradients vanish, due to the repetitive use of softmax which saturates.</d-footnote> to the router embeddings (akin to Mixtral) and the
processing step is complete.<d-footnote>I have experimented with a setup where
the router also performs a GNN message passing step, but found that it gave no
substantial difference</d-footnote>
...
out-of-distribution (80). The first one is 100 samples, generated using the
...
3080 for inference.</d-footnote>

### Poor hint prediction

| Approach | `i` | `s` | `k` | `q` | `pi` | `phase` |
| :------------- |--------:| -----:| -----:| -----:| -----:| -----:|
| NAR | 84.97% | <span style="color:#EA7125">30.38%</span> | 99.84% | 86.88% | <span style="color:#EA7125">99.92%</span> | 99.53%
| MixNAR | <span style="color:#D6083B">25.04%</span> | 34.38% | 97.12% | <span style="color:#D6083B">31.82%</span> | 95.42% | 99.97%
...
step is mispredicted. Test dataset is still the official one.
...
are of course self-generated and not on downloaded from the officially
...
start moving `s`, when `i` has done `|P|` steps.
...
# So is the boss finally dead? (Outro)
...
definition is "legal" and to the reviewers to decide if modularity is cool
idea.
