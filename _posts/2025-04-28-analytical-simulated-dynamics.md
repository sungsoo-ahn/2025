...
$$ \mathcal{L}^{u} = \frac{1}{2B} \sum_{i=1}^{B} \left( \hat{y}_{i}^{u} - y_{i}^{u} \right)^{2}, $$
...
$$ \begin{align} 
\mathcal{L^{u}} = & \frac{1}{2B} \sum_{i=1}^{B} \left( \hat{y}_{i}^{u} - y_{i}^{u} \right)^{2} \\
 = & \frac{1}{2B} \sum_{i=1}^{B} \left( \sum_{k=1}^{K} g\left( \frac{W_{k}x_{i}^{u}}{\sqrt{N}} \right) - \sum_{k'=1}^
{M} g\left( \frac{W^{*}_{k'}x_{i}^{u}}{\sqrt{N}} \right) \right)^{2} \\
= & sadasd \\
\end{align} $$
