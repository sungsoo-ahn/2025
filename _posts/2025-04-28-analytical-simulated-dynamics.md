...
$$ 
y_{s}^{u} = \sum_{m=1}^{M} g\left( \frac{W^{*}_{m} x_{s}^{u}}{\sqrt{N}} \right) + \sigma \xi^{u} \hspace{0.3cm} \text{ and } \hspace
{0.3cm} 
...
and $K$ 
are the number of 
neurons in the teacher and student networks, respectively. From here on, neuron indexing will be $i, j, k$ for the 
...
{M} g\left( \frac{W^{*}_{m}x_{s}^{u}}{\sqrt{N}} \right) + \sigma \xi^{u} -\sum_{k=1}^{K} g\left( \frac{W_{k}x_{s}^{u}}{\sqrt{N}} 
\right) \right]^{2} \\
\frac{\partial \mathcal{L}}{\partial W_{i}} = & \frac{1}{B}\sum_{s=1}^{B} \left[\sum_{m=1}^
{M} g\left( \frac{W^{*}_{m}x_{s}^{u}}{\sqrt{N}} \right) -\sum_{k=1}^{K} g\left( \frac{W_{k}x_{s}^{u}}{\sqrt{N}} 
...
$$ 
R = \frac{W^{*}W^{T}}{N} \hspace{0.3cm} \text{ and } \hspace
{0.3cm} 
Q = \frac{W W^{T}}{N}.
$$
