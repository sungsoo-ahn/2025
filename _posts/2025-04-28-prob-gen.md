...
As a proof of concept to validate this approach and quantify its ability to expand beyond evolutionary constraints, we conducted a comparative analysis of protein sequence generation using 100 samples each from standard diffusion and our next-generation approach. 
...
The TM-score and pLDDT metrics measure different aspects of structural quality - TM-score indicates global fold similarity while pLDDT reflects local structural confidence. The small decrease in these metrics (TM-score: 0.85→0.82, pLDDT: 90.0→87.0) suggests our approach maintains structural integrity even while venturing into novel sequence space. For context, TM-scores above 0.8 typically indicate very high structural similarity, while pLDDT scores above 85 are considered highly reliable predictions.

This increased exploration also resulted in the discovery of 28 unique structural motifs compared to 12 in the standard approach which is particularly promising, as novel structural motifs often correspond to new functional capabilities in proteins. This suggests our approach isn't just generating random variations, but is finding meaningful new arrangements that could have functional significance.

Notably, our method maintained structural viability even at higher evolutionary distances by employing a dynamic weighting scheme that balances exploration with structural constraints. The distribution of evolutionary distances indicates how our approach shifts the sampling distribution toward more novel sequences while maintaining the total probability mass, indicating efficient exploration of previously unexplored protein space.
